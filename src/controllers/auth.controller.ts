...
export class AuthController {
  ...
  public resendVerifyEmail = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!email) throw new HttpException(false, 400, "Email is required");

    await this.auth.resendVerifyEmail(email);
    res.status(200).json(apiResponse(200, "OK", "Email has been sent", {}));
  });
}